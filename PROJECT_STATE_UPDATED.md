...
A full Contact Page feature has been built following the same
Page → Data → Mapper → Layout → Components pattern used by the About page.
...
- `contact.data.ts` exports `getContactData()` as the single raw content
  source (hero copy, contact options, project-fit items, project type /
  budget range labels, response-time copy).
- `contact-mapper.ts` exports `mapContactData()`, which composes raw data
  into a `ContactViewModel`. It also converts plain string labels
  (project types, budget ranges) into `SelectOption[]` (`{ id, label, value }`)
  so components never hardcode form option data.
- `contact-layout.tsx` is composition-only — it orders
  `ContactHero → ContactOptions → ContactFit → ContactForm` and contains
  no data fetching or business logic.
- `contact-motion-wrapper.tsx` is a `"use client"` Framer Motion wrapper
  reused across all sections for scroll-reveal, mirroring
  `about-motion-wrapper.tsx`.
- `contact-form.tsx` is its own `"use client"` component (not just wrapped
  by the motion wrapper) because it owns controlled form state and a
  submit handler — this is a deliberate, scoped exception to the
  "one client component" pattern used on the About page, since the form's
  interactivity is independent of motion.
- `contact-options.tsx` avoids `lucide-react` brand icons (Github /
  Linkedin) — which previously caused export errors in this project — and
  uses small inline SVG icon functions instead.
- The contact form is frontend-only: it has full client-side validation
  (`required` fields, `type="email"`) and a local submit state, but does
  not call any API route or send email yet. It is structured so an
  `/api/contact` route using Zod validation + Resend/Nodemailer can be
  dropped in later without changing the component's props.
- `page.tsx` only calls `getContactData()` → `mapContactData()` → renders
  `<ContactLayout data={data} />`, per the Definition of Done.

### Verification

- `npx tsc --noEmit` passes with `strict`, `noUnusedLocals`, and
  `noUnusedParameters` enabled against the new files — no `any`, no
  unused imports.
- Still pending in the real project: run `npm run build` from the project
  root to confirm the new route compiles inside the full Next.js app
  (this sandbox doesn't have the actual project installed, only an
  isolated TypeScript check of the new files).

### Assumptions made

- Email/GitHub/LinkedIn placeholder values
  (`hello@odayzoheer.dev`, `github.com/odayzoheer`,
  `linkedin.com/in/odayzoheer`) — replace with real links.
- Background color `#05070d` used for `<main>` to match the existing dark
